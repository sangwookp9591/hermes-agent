...
- 상태 **Accepted (2026-09-04 4차 개정 — 품질 축 측정 완료)**
- 날짜 2026-09-04
...
| 작업 | 배치 |
|---|---|
| 파일/코드 탐색, 나열 (틀리면 즉시 드러나는 일) | luna / low |
| 단순 수정, CRUD/DTO, 테스트 코드 | luna / medium |
| 일반 구현, 빌드·테스트 오류 수정 | luna / medium |
| **계산·집계·정합성이 섞인 모든 작업** | **luna / high** (§4.9 — low는 조용히 틀린다) |
| 여러 파일 수정, 복잡한 비즈니스 로직 | luna / high |
| 복잡한 버그, Root Cause, 동시성/트랜잭션 | sol / high |
| Production 장애, 반복 실패 | sol / high (max는 근거 확보 전까지 쓰지 않는다 §4.9) |
...
## 4.9 품질 축 — `low`는 실제로 틀린다, `max`는 이득이 없다

앞선 측정(§4.6~4.7)은 전 조건 정답이라 **품질 차이를 못 봤다.**
실패가 나오는 난이도로 재측정했다.

**방법**: 정답을 brute force로 확정한 문제 5종. `writer` 프로필 사용 —
`terminal`/`code_execution`이 없어 **코드 실행 없이 순수 추론**만 가능하다.

| | 정답 | low | high | max |
|---|---|---|---|---|
| P1 부분집합 합 ≡0 (mod 5) | 820 | 1640 ✗ | 820 ✓ | 820 ✓ |
| P2 D(10) 교란순열 | 1334961 | ✓ | ✓ | ✓ |
| P3 1..10000 중 2,3,5,7 비배수 | 2285 | 2284 ✗ | 2285 ✓ | 2285 ✓ |
| P4 n \| 2ⁿ−1 (n≤2000) | 1 | ✓ | ✓ | ✓ |
| P5 10⁷ 미만 팰린드롬 ∩ 7의 배수 | 1588 | 1591 ✗ | 1588 ✓ | — |
| **정답률** | | **2/5 (40%)** | **5/5 (100%)** | **4/4 (100%)** |
| **평균 토큰** | | 813 | 1666 | 1162 |

**판정**

- **`low → high`: 실질 이득.** 40% → 100%. 토큰 2배로 정답률이 2.5배가 된다.
  P1은 2배 틀리고(1640 vs 820) P3·P5는 근소하게 빗나간다 —
  **`low`의 오답은 "거의 맞는 값"이라 눈으로 못 걸러낸다.** 이게 위험하다.
- **`high → max`: 이득 없음.** 둘 다 100%인데 `max`가 토큰을 **더 적게** 썼다(1162 < 1666).
  §4.7의 "상단이 무너진다"가 품질 축에서도 재확인됐다.

> **결론**: 사다리의 유효 구간은 **`low → high`** 하나다.
> `max`는 근거가 나오기 전까지 쓰지 않는다.
> `low`는 **검증 가능한 단순 작업에만** 쓴다 — 탐색·나열처럼 틀려도 즉시 드러나는 일.
> 계산·집계가 섞이면 `low`는 조용히 틀린다.

**한계**: 문제 5종·조건당 1회. 모델은 luna 한 종.
P5의 `max`는 응답 파싱 실패로 결측이다.
재현 데이터: `docs/bench.json`(문제+정답), `docs/bench-luna.csv`.

## 미검증 — 실행 전 확인할 것

1. ~~spark `reasoning_effort` 차등~~ → **해소(§4.6, §4.7).** `xhigh` 무효 확정.
2. ~~quota 회복 후 Luna/Sol + `max` 재측정~~ → **해소(§4.7).** 기본 모델을 luna로 변경.
3. ~~정답률이 갈리는 난이도에서의 effort 효과~~ → **해소(§4.9).**
   `low` 40% / `high` 100% / `max` 100%. 유효 구간은 `low → high` 하나다.
4. **ChatGPT 구독 인증이 동시 워커 N개에서 rate limit에 걸리는 지점.**
   Kanban은 프로필별 OS 프로세스라 동시성이 올라간다.
5. ~~`--goal` 루프가 AGENTIC 작업을 완주하는가~~ → **해소.**
   "구현 → 테스트 작성 → 실행 → 통과까지" 카드에서 워커가 pytest 부재를 만나
   **스스로 unittest로 전환해 완주**했다. 독립 실행 결과 5/5 통과.
   (브라우저 왕복이 본질인 작업은 여전히 미검증.)
